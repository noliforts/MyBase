...
40 тестов покрывают: лексер, парсер, все виды SELECT (ORDER BY, LIMIT, OFFSET,
агрегаты, GROUP BY), транзакции, персистентность, импорт и экспорт (CSV),
а также многопоточную вставку.
